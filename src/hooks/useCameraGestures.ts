...
export const CAMERA_MODE_EVENT = "dinodash-camera-mode-changed";
...
export const CAMERA_ZOOM_LEVELS = CAMERA_ZOOM_PERCENT_LEVELS.map((percent) => CAMERA_BASELINE_ZOOM_FACTOR / (percent / 100));
...
export const useCameraGestures = ({
  ...
  useEffect(() => { isMobilePortraitRef.current = isMobilePortrait; }, [isMobilePortrait]);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    setCameraOffset({ x: dragOffsetStart.x - deltaX * sensitivity, z: dragOffsetStart.z - deltaY * sensitivity });
  };
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    if (e.touches.length >= 2) {
      ...
      twoFingerGestureRef.current = { startDist: dist, startMidX: midX, startMidY: midY, lastDist: dist, lastMidX: midX, lastMidY: midY, intent: e.touches.length >= 3 ? "pan" : null };
      return;
    }
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length >= 2) {
      ...
      if (gesture.intent === null) {
        ...
        if (distDelta > 10) gesture.intent = "pinch"; else if (midDelta > 12) gesture.intent = "pan";
      }
      if (gesture.intent === "pinch") {
        const delta = newDist - gesture.lastDist;
        if (Math.abs(delta) >= PINCH_ZOOM_STEP_DISTANCE_PX) {
          setCameraZoomIndex((idx) => Math.max(fitToWidthZoomIndex, Math.min(CAMERA_ZOOM_LEVELS.length - 1, delta > 0 ? idx + 1 : idx - 1)));
          gesture.lastDist = newDist;
        }
      } else if (gesture.intent === "pan") {
        const sensitivity = 0.1; const dx = midX - gesture.lastMidX; const dy = midY - gesture.lastMidY;
        setCameraOffset((prev) => isMobilePortrait ? { x: prev.x + dy * sensitivity, z: prev.z - dx * sensitivity } : { x: prev.x - dx * sensitivity, z: prev.z - dy * sensitivity });
      }
      gesture.lastMidX = midX; gesture.lastMidY = midY; pinchDistanceRef.current = newDist; return;
    }
    if (isDragging && e.touches.length === 1) {
      const touch = e.touches[0]; const deltaX = touch.clientX - dragStart.x; const deltaY = touch.clientY - dragStart.y; const sensitivity = 0.1;
      setCameraOffset(isMobilePortrait ? { x: dragOffsetStart.x + deltaY * sensitivity, z: dragOffsetStart.z - deltaX * sensitivity } : { x: dragOffsetStart.x - deltaX * sensitivity, z: dragOffsetStart.z - deltaY * sensitivity });
    }
  };

  const handleTouchEnd = (e: React.TouchEvent) => {
    if (e.touches.length === 0) { multiTouchActiveRef.current = false; setIsDragging(false); twoFingerGestureRef.current = null; pinchDistanceRef.current = null; }
    else if (e.touches.length < 2) { twoFingerGestureRef.current = null; pinchDistanceRef.current = null; setIsDragging(false); }
  };
  const handleDoubleClick = () => {
    if (viewMode === "fps" || viewMode === "sprite" || viewMode === "top") return;
    setCameraOffset({ x: 0, z: 0 }); onPushHudMessage("View reset");
  };

  return {
    cameraOffset, setCameraOffset, cameraZoomIndex, setCameraZoomIndex, cameraZoomFactor, cameraZoomPercent,
    canZoomIn, canZoomOut, fitToWidthZoomIndex, userZoomTouched, setUserZoomTouched,
    cameraMode, setCameraMode, isDragging, gestureSurfaceRef,
    handlers: { onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseLeave, onTouchStart: handleTouchStart, onTouchMove: handleTouchMove, onTouchEnd: handleTouchEnd, onDoubleClick: handleDoubleClick },
  };
};
